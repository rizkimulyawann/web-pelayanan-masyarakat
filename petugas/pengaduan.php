...
if (isset($_POST['tanggapi'])) {
    ...
    $petugasId = $_SESSION['data']['id_petugas'];
      // Ambil nilai status dari formulir

    // Simpan tanggapan ke tabel tanggapan
	$testing = mysqli_real_escape_string($koneksi, $_POST['testing']);
    $query = mysqli_query($koneksi, "INSERT INTO tanggapan VALUES (NULL, '" . $r['id_pengaduan'] . "', '" . $tgl . "', '" . $tanggapan . "', '" . $petugasId . "')");

    if ($query) {
        // Perbarui status di tabel pengaduan sesuai nilai yang dipilih dari dropdown
        ...
        if ($update) {
            ...
            echo mysqli_error($koneksi); // Tambahkan ini untuk melihat pesan kesalahan SQL
        }
    } else {
